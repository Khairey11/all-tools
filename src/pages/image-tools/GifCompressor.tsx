...
export const GifCompressor = () => {
    ...
    const [manualFps, setManualFps] = useState(0); // 0 = original
    ...
    // state — they never cancel the run. (A previous version cancelled
    // itself on every re-render and froze at "Decoding GIF…".)
    const processJob = useCallback(
        async (jobId: string, original: File, cachedDecoded?: DecodedGif) => {
            ...
            try {
                ...
                const onProgress = (p: number, label?: string) => {
                    if (mountedRef.current) {
                        patchJob(jobId, { progress: p, label: label ?? 'Working…' });
                    }
                };
                ...
                if (runMode === 'target' && !result.targetMet) {
                    toast(
                        `Couldn't reach ${formatFileSize(targetBytes)} for "${original.name}" — best result shown (${formatFileSize(result.bytes)}).`,
                        { icon: '⚠️' }
                    );
                }
        ...
        [mode, targetBytes, manualColors, manualFps, manualScale, manualLossy, patchJob]
    );
    ...
    const settingsKey =
        mode === 'target'
            ? `t:${targetBytes}`
            : `m:${manualColors}:${manualFps}:${manualScale}:${manualLossy}`;
        ...
        <div className="space-y-8 py-4 animate-fade-in">
            <header className="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-text-muted">
                        Compress heavy animated GIFs to your exact target size — 100% in your browser
                    </p>
                </div>
            </header>
            ...
            {!supportedRef.current && (
                <div className="p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl text-sm">
                    ⚠️ Your browser doesn't support animated GIF decoding (WebCodecs).
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-xs text-primary/80 font-bold pt-2">
                                ✨ Keeps animation & transparency · Set an exact target size
                            </p>
                        </div>
                    </div>
                </motion.div>
                ...
                <>
                        ...
                        {mode === 'target' && (
                            <div className="space-y-3">
                                ...
                                <p className="text-xs text-text-muted">
                                    The engine tries gentle steps first (colors → frame rate → resolution)
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="text-text font-mono text-xs bg-white/5 px-2 py-1 rounded">
                                            {manualFps === 0 ? 'Original' : `${manualFps} fps`}
                                        </span>
                                    </div>
                                    <input
                                        type="range" min={0} max={30} step={1}
                                        value={manualFps}
                                        onChange={(e) => setManualFps(Number(e.target.value))}
                                        className="w-full h-2 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary"
                                    />
                                    <div className="flex justify-between text-xs text-text-muted/40">
                                        <span>Original</span><span>30 fps</span>
                                    </div>
                                </div>
                                ...
                                <p className="sm:col-span-2 text-xs text-text-muted">
                                    💡 Changes apply automatically after a short pause — no button needed.
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    {savings > 0 && (
                        <div className="p-4 bg-gradient-to-r from-primary/10 to-purple-500/10 border border-primary/20 rounded-2xl text-sm text-text font-semibold flex items-center gap-3">
                            ...
                            Saved {savings}% overall — {formatFileSize(totalOriginal)} →{' '}
                            ...
                            >
                                <div className="flex items-center justify-between gap-3 flex-wrap">
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-xs text-text-muted">
                                            {job.decoded
                                                ? `${job.decoded.width}×${job.decoded.height} · ${job.decoded.frameCountTotal} frames`
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-xs font-bold uppercase tracking-widest text-primary">
                                                ...
                                                % smaller)
                                                {mode === 'target' &&
                                                    (job.result.targetMet ? ' · target met ✓' : ' · best effort')}
                                                {job.result.lossless && ' · lossless ⚡'}
                                            </p>
                                            <img
                                            ...
                                            {job.result.passes.length > 1 && (
                                                <p className="text-xs text-text-muted">
                                                    Tried {job.result.passes.length} quality steps · final:{' '}
                                                    {job.result.width}×{job.result.height} · kept ~
